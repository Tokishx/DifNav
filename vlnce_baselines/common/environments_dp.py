...
@baseline_registry.register_env(name="VLNCEDaggerEnv-1-DP")
class VLNCEDaggerEnv(habitat.RLEnv):
    ...
    def get_observation(self,
        ...
        old_agent_state = self._env.sim.get_agent_state()
        obs = self._env.sim.get_observations_at(source_position, source_rotation, keep_agent_at_new_pose)
        sim = self._env.sim
        agent_rotation = sim.get_agent(0)._sensors['rgb'].node.absolute_transformation()
        array = np.array([[agent_rotation[i][j] for j in range(4)] for i in range(4)], dtype=np.float32)
        ex = array.T
        ex_z = ex[3,1]
        ex_y = ex[3,2]
        ex[3,2] = ex_z
        ...
        self._env.sim.get_observations_at(old_agent_state.position,old_agent_state.rotation, keep_agent_at_new_pose)
        return obs, ex 
    ...
    def dagger_cands(self, forward, cand_nums=12):
        ...
        for cand_angle in range(cand_nums):
            angle = (cand_angle / cand_nums) * 2 * np.pi
            ...
            for k in range(ksteps):
                sim.step_without_obs(forward_action)
                post_state = sim.get_agent_state()
            cand_real_pos = post_state.position
            cand_poses.append(cand_real_pos)
            # reset agent state
            sim.set_agent_state(init_state.position, init_state.rotation)
        return cand_poses

    def dagger_dist_to_ref(self, ref_path, waypoint_spacing):
        episode_id = self._env.current_episode.episode_id
        if episode_id != self.prev_episode_id:
            self.progress = 0
            self.prev_sub_goal_pos = [0.0, 0.0, 0.0]
        ...
        # dagger dis to subgoal
        # select the circle which radius = ws * 0.25m, and average segment for 12/36/72 parts
        # add the dagger vp
        # generate dagger vp in the environment(bound with curr_pos)
        cand_interventions = self.dagger_cands(waypoint_spacing * 0.25)
        dagger_dists_to_subgoal = []
        
        for dagger_pos in cand_interventions:
            dist = self._env.sim.geodesic_distance(dagger_pos, sub_goal_pos)
            dagger_dists_to_subgoal.append(dist)

        oracle_dagger_pos = cand_interventions[np.argmin(dagger_dists_to_subgoal)]
        self.prev_episode_id = episode_id
            
        return oracle_dagger_pos
    ...
    def single_step_control(self, pos, tryout, vis_info, waypoint_spacing):
    ...
    def step(self, action, vis_info, waypoint_spacing, *args, **kwargs):
        act = action['act']
        exec_ations = action['exec_ations']
        if act == 4: # high to low

            # 1. back to stop node
            gmap_observations = []
            agent_states = []
            if isinstance(exec_ations, list):
                self.single_step_control(exec_ations[0],action['tryout'], vis_info, waypoint_spacing)
            else:
                if self.video_option:
                    self.get_plan_frame(vis_info)
                # action sequence
                # save all obs and agent_state
                for action_pos in exec_ations:
                    # cant move successfully
                    self.single_step_control(action_pos,action['tryout'], vis_info, waypoint_spacing)
                    agent_state = self._env.sim.get_agent_state()
                    observation = self.get_observation_at(agent_state.position, agent_state.rotation)
                    
                    gmap_observations.append(observation)
        ...
        if act == 4 and not isinstance(exec_ations, list):
            observations = gmap_observations

        # return observations, reward, agent_states, done, info
        return observations, agent_states, done, info
